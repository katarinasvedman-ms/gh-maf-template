...
17. ContractCoverageValidator does not check mode coverage. AllowedModes are enforced at runtime by AllowListToolPolicy; scenarios have no mechanism to declare or control execution mode. A mode-check predicate based on tool name alone (the previous implementation) is silently wrong and was removed.
